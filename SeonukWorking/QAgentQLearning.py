...
class QAgentQLearning():
    ...
    def get_action(self, state):
        q_table = np.load(
            "C:/OSSP2/OSSP2/Seonuk-Working/100000_dumbagent_save.npy")

        action_val = q_table[self.env.get_state(self.agent_num)[1][0][0], self.env.get_state(
            self.agent_num)[1][0][1], self.wall_num_prime, :]
        action = np.argmax(action_val)
...
def main():
    ...
    for _ in range(100000):  # 총 100000 에피소드 동안 학습
        ...
        dumbagent_2 = HeuristicAgent(env, agent_2)
        walk = 0

        while done == 0:  # 한 에피소드가 끝날 때 까지
            while True:
                a_1 = agent1.select_action(env.get_state(
            ...
            agent1.update_table(env.get_state(agent_1),
                                a_1, r_1, s_prime_1, agent_1)
