...
def main():
    ...
    oldmousex = 0  # used to store x coordinate of mouse event
    oldmousey = 0  # used to store y coordinate of mouse event
    ...
    while True:  # main game loop
        ...
        for event in pygame.event.get():  # event handling loop
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
                ...
                oldmousex, oldmousey = event.pos
                oldmousey = WINDOWHEIGHT - oldmousey - 1
                mouseDowned = True
...
def getObjAtPixel(oldx, oldy, newx, newy):
    dir_left = False
    dir_up = False
    ...
    for boxx in range(BOARDWIDTH):
        for boxy in range(BOARDHEIGHT):
            ...
            if boxRect.collidepoint(x, y):
                return (boxx, boxy, 0)  # 0 for boxes
            boxRect = pygame.Rect(left, top, BOXSIZE+GAPSIZE, BOXSIZE+GAPSIZE)
            if boxx < (BOARDWIDTH - 1) and boxy < (BOARDHEIGHT - 1):
                if boxRect.collidepoint(x, y):
                    return (boxx, boxy, 1)  # 1 for walls
    return (None, None, 3)  # 3 for nothing
