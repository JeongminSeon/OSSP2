...
class QAgentQLearning():
    def __init__(self, width=5):
        ...
        # agent1의 q벨류를 저장하는 변수. 모두 0으로 초기화.
        self.q_table_1 = np.zeros(
            (width, width, self.wall_act_size, 4 + (width-1) * (width-1) * 2))
        # agent2의 q벨류를 저장하는 변수. 모두 0으로 초기화.
        self.q_table_2 = np.zeros(
            (width, width, self.wall_act_size, 4 + (width-1) * (width-1) * 2))
        ...
        if (agent_num != AGENT_1):
            ...
            if coin < self.eps:
                action = random.randint(
                    0, 3 + (self.width-1) * (self.width-1) * 2)
            else:
                action_val = self.q_table_1[state[1][0][0],
                                            state[1][0][1], self.wall_num_prime, :]
    ...
    def update_table(self, state, a, r, s_prime, agent_num):
        ...
        if (agent_num != AGENT_1):
            ...
            self.q_table_2[state[1][1][0], state[1][1][1], self.wall_num, a] = self.q_table_2[state[1][1][0], state[1][1][1], self.wall_num, a] + self.alpha * (
                r + np.amax(self.q_table_2[s_prime[1][1][0], s_prime[1][1][1], self.wall_num_prime, :]) - self.q_table_2[state[1][1][0], state[1][1][1], self.wall_num, a])
            ...
            if a in range(4, 36):
                for i in range(self.wall_act_size):
                    if self.wall_states == self.all_wall_states[i]:
                        self.wall_num_prime = i
            # QLearning 업데이트 식을 이용
            self.q_table_1[state[1][0][0], state[1][0][1], self.wall_num, a] = self.q_table_1[state[1][0][0], state[1][0][1], self.wall_num, a] + self.alpha * (
                r + np.amax(self.q_table_1[s_prime[1][0][0], s_prime[1][0][1], self.wall_num_prime, :]) - self.q_table_1[state[1][0][0], state[1][0][1], self.wall_num, a])
...
def main():
    ...
    for _ in range(1000):  # 총 1,000 에피소드 동안 학습
        ...
        while done == 0:  # 한 에피소드가 끝날 때 까지
            while True:
                a_1 = agent.select_action(env.get_state(
                    agent_1), agent_1)  # agent1 가능한 action 선택

                if a_1 in env.get_legal_action(env.get_state(agent_1)):
                    break

            # action 진행 후 state, reward, done 반환
            s_prime_1, r_1, done = env.step(agent_1, a_1)
            agent.update_table(env.get_state(agent_1), a_1,
                               r_1, s_prime_1, agent_1)
    ...
    np.save('Seonuk-Working/agent1', agent.q_table_1)
