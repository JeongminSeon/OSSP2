...
class QAgentSARSA():
    ...
    def update_table(self, state, a, r, s_prime, agent_num):
        ...
        if (agent_num != AGENT_1):
            ...
            a_prime = self.select_action(s_prime,agent_num) # s'에서 선택할 액션 (실제로 취한 액션이 아님)
            #SARSA 업데이트 식을 이용
            self.q_table_2[state[1][1][0],state[1][1][1],a] = self.q_table_2[state[1][1][0],state[1][1][1],a] + self.alpha * (r + self.q_table_2[s_prime[1][1][0],s_prime[1][1][1],a_prime] - self.q_table_2[state[1][1][0],state[1][1][1],a])
            
        else:
            #state, a, r, s_prime = transition
            a_prime = self.select_action(s_prime,agent_num) # s'에서 선택할 액션 (실제로 취한 액션이 아님)
            #SARSA 업데이트 식을 이용
            self.q_table_1[state[1][0][0],state[1][0][1],a] = self.q_table_1[state[1][0][0],state[1][0][1],a] + self.alpha * (r + self.q_table_1[s_prime[1][0][0],s_prime[1][0][1],a_prime] - self.q_table_1[state[1][0][0],state[1][0][1],a])
